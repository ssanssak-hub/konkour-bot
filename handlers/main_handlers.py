...
# ایمپورت هندلرهای مدیریت
from handlers.admin_handlers import admin_panel_handler, reminder_management_handler
...
# =============================================================================
# ثبت هندلرهای اصلی منو - با دسترسی ادمین
# =============================================================================

# هندلرهای منو
@dp.message(F.text == "🔙 بازگشت به منوی اصلی")
async def back_to_main_wrapper(message: types.Message):
...
# هندلرهای مدیریت - فقط برای ادمین
@dp.message(F.text == "⚙️ پنل مدیریت")
async def admin_panel_wrapper(message: types.Message):
    await admin_panel_handler(message)

@dp.message(F.text == "⏰ مدیریت یادآوری‌ها")
async def reminder_management_wrapper(message: types.Message):
    await reminder_management_handler(message)

# هندلرهای ریمایندر پیشرفته - فقط برای ادمین
